...
def fetch_private_collection_quick(
    ...
    try:
        ...
        create_error(
            status_code=500, error_code="INTERNAL_ERROR", detailed_message=str(e)
        )
    finally:
        import_lock.release()
...
def update_similar_games_endpoint(
    ...
    try:
        similarities = update_similar_games(max_similar_games=10)
        return {
            "message": "Similar games updated successfully.",
...
def reset_available_for_all_games(
    ...
    updated_rows = db.query(Game).update(
        {Game.available: Game.quantity}, synchronize_session=False
    )
    db.commit()
    return {
