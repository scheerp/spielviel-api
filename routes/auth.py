...
def login_for_access_token(
    ...
    if not user or not verify_password(form_data.password, user.hashed_password):
        create_error(
            status_code=status.HTTP_401_UNAUTHORIZED, error_code="NOT_AUTHORIZED"
        )
...
def register_user(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    if request.invite_code != required_invite:
        create_error(
            status_code=status.HTTP_403_FORBIDDEN, error_code="INVALID_INVITE_CODE"
        )

    # E-Mail-Validierung
    if not re.match(EMAIL_REGEX, request.email):
        create_error(
            status_code=status.HTTP_400_BAD_REQUEST, error_code="INVALID_EMAIL"
        )
    ...
    if not re.match(PASSWORD_REGEX, request.password):
        create_error(
            ...
            detailed_message="Passwort muss mindestens 6 Zeichen lang sein,"
            "eine Zahl und einen Großbuchstaben enthalten.",
        )
    ...
    if existing_user or existing_email:
        create_error(
            status_code=status.HTTP_409_CONFLICT, error_code="USER_ALREADY_EXISTS"
        )
