...
from models import (
    ...
    User,
    GameResponse,
    UserGameKnowledgeRequest,
    GameExplainersResponse,
    Game,
)
...
def update_game_familiarity(
    ...
    if record:
        ...
        record = UserGameKnowledge(
            user_id=current_user.id, game_id=game_id, familiarity=request.familiarity
        )
        db.add(record)
...
def get_game_explainers(
    ...
    explainer_groups = [
        ...
        for familiarity, users in sorted(
            explainer_groups_dict.items(), key=lambda x: x[0], reverse=True
        )
    ]
